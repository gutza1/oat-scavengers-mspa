...
export default class GameScene extends Phaser.Scene {
	...
	create() {
		...
		for (var i = 0; i < dialogues_json.length; i++) {
            window.gameState.dialogues.set(
                dialogues_json[i].id,
                dialogues_json[i]
            );
        }
		...
		for (var i = 0; i < world_objects_json.length; i++) {
			...
			for (var j = 0; j < item_effects_data.length; j++) {
				item_effects.set(item_effects_data[j].item, item_effects_data[j].effects);
			}
		...
		for (var i = 0; i < rooms_json.length; i++) {
			const room_objects:WorldObject[] = rooms_json[i].world_objects.map((obj_id:string) => window.gameState.worldObjects!.get(obj_id));;
			const newRoom = new Room(rooms_json[i].id, rooms_json[i].background, room_objects);

            window.gameState.rooms.set(
                items_json[i].id,
                newRoom
            );
        }
